...
class LeakyReluLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        For inputs `x` and outputs `y` this corresponds to `αx if x ≤ 0 && x if x > 0`.
        """
        outputs = np.maximum(inputs, 0.01 * inputs)
        return outputs
...
class ELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        For inputs `x` and outputs `y` this corresponds to `y = max(0, x)`.
        """
        outputs = inputs * (inputs > 0) + (inputs <= 0) * (np.exp(inputs) - 1)
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        #gradients = grads_wrt_outputs * (outputs > 0) + (outputs <= 0) * (self.fprop(inputs) + 1) * grads_wrt_outputs
        gradients = grads_wrt_outputs * (outputs > 0) + (outputs <= 0) * np.exp(inputs) * grads_wrt_outputs
        return gradients
